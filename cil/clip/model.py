...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, text_or_image=None):
        ...
        self.old_adapter = None  # track old adapter for task adaptation
    ...
    def forward(self, x: torch.Tensor):
        ...
        batch_size, n_ctx, d_model = x.shape
        x = x.reshape(batch_size * n_ctx, d_model)
        x = self.adapter(x)
        x = x.reshape(batch_size, n_ctx, d_model)
        return x
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, text_or_image=None):
        ...
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask, text_or_image) for _ in range((3 * layers) // 2)])
